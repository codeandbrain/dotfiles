...
**What it is:** Multi-provider terminal agent — works with Claude, local Ollama, or other providers, all free.**Binary:** Already installed at `~/.local/bin/opencode`**Run:** `opencode` (alias: `oc`)
...
**Open:** [http://localhost:3000](http://localhost:3000)**Note:** Pin the version tag — never use `:latest` or `:main`. Check [https://github.com/open-webui/open-webui/releases](https://github.com/open-webui/open-webui/releases) before bumping.

---

## 5. Continue (VS Code extension )

**Install in VS Code:** Extensions panel → search `Continue`**Config:** `~/.continue/config.json` — switch between Claude, local Ollama, or others by editing one field.

---

## 6. Cline (VS Code extension)

**Install:** Extensions panel → search `Cline`**Uses:** Claude Sonnet via API key — free tier available.
...
**To configure MCP in Claude Code:** Edit `~/.claude/mcp_settings.json` (created automatically on first run).**To configure MCP in Continue:** Edit `~/.continue/config.json` under the `mcpServers` key.
